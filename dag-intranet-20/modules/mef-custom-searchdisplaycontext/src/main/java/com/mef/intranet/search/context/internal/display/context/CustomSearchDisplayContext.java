...
public class CustomSearchDisplayContext implements SearchDisplayContext {
	
	private final Log _log = LogFactoryUtil.getLog(CustomSearchDisplayContext.class);
	
	//public static final String DEFAULT_CUR_PARAM = "cur";
	//public static final int MAX_DELTA = 10;
	@Deprecated
	public CustomSearchDisplayContext(RenderRequest renderRequest, RenderResponse renderResponse, PortletPreferences portletPreferences)
	throws PortletException {
		
		this(renderRequest, renderResponse, portletPreferences, PortalUtil.getPortal(), HtmlUtil.getHtml(), LanguageUtil.getLanguage(),
		null, new IndexSearchPropsValuesImpl(), new PortletURLFactoryImpl());
	}
	
	public CustomSearchDisplayContext(RenderRequest renderRequest, RenderResponse renderResponse, PortletPreferences portletPreferences,
		Portal portal, Html html, Language language, FacetedSearcherManager facetedSearcherManager,
		IndexSearchPropsValues indexSearchPropsValues, PortletURLFactory portletURLFactory)
	throws PortletException {
		...
		try {
			...
			String emptyResultMessage = language.format(request, "no-results-were-found-that-matched-the-keywords-x",
			"<strong>" + html.escape(keywords) + "</strong>", false);
			SearchContainer<Document> searchContainer = new SearchContainer<>(_renderRequest, null, null, DEFAULT_CUR_PARAM, MAX_DELTA,
			getPortletURL(), null, emptyResultMessage);
			FacetedSearcher facetedSearcher = facetedSearcherManager.createFacetedSearcher();
			List<Long> categoriesIds = ListUtil.toList(getAreeTematiche());
			//		Long[] areeTem=getAreeTematiche();
			//
			//		List<Long> categoriesIds=new ArrayList<>();
			//
			//		if(getAreeTematiche()!=null){
			//			categoriesIds=Arrays.asList(getAreeTematiche());
			//		}
			...
			String facetConfig = getSearchConfiguration();	
			
//			_log.info("facetConfig: "+facetConfig);
			
			JSONObject jsonFacet = JSONFactoryUtil.createJSONObject(facetConfig);
			JSONArray arrayFacet = JSONFactoryUtil.createJSONArray(jsonFacet.getString("facets"));
			JSONObject jsonData = null;		
			JSONArray arrayData = null;
			
			for(int i = 0; i<arrayFacet.length(); i++){
				if(("entryClassName").equalsIgnoreCase(arrayFacet.getJSONObject(i).getString("fieldName"))){
					 jsonData = JSONFactoryUtil.createJSONObject(arrayFacet.getJSONObject(i).getString("data"));
					  arrayData = JSONFactoryUtil.createJSONArray(jsonData.getString("values"));	
					 break;
				}
				
			}
			if(arrayData!=null && arrayData.length()>0){
			 String[] entryClassNames = new String[arrayData.length()];
			 for(int j =0;j<arrayData.length();j++){
				 entryClassNames[j] = arrayData.getString(j);
//				 _log.info("className: "+entryClassNames[j]);
			 }
		      searchContext.setEntryClassNames(entryClassNames);
			}
		      
			if (!categoriesIds.isEmpty()) {
				BooleanQuery booleanQueryCategoryId = createBooleanQueryCategories(searchContext, categoriesIds, BooleanClauseOccur.SHOULD);
				BooleanClause booleanClauseCategoryId = BooleanClauseFactoryUtil.create(searchContext, booleanQueryCategoryId,
	...
	public String checkViewURL(String viewURL, String currentURL) {
		...
		String url = viewURL;
		if (Validator.isNotNull(url) && url.startsWith(themeDisplay.getURLPortal())) {
			
			url = HttpUtil.setParameter(url, "inheritRedirect", isViewInContext());
			if (!isViewInContext()) {
				url = HttpUtil.setParameter(url, "redirect", currentURL);
			}
		}
		return url;
		//		} catch(Exception e) {
		//		throw new SearchContextRuntimeException(e);
		//		}
	}
	...
	public int getCollatedSpellCheckResultDisplayThreshold() {
		...
		int collatedSpellCheckResultScoresThreshold = _indexSearchPropsValues.getCollatedSpellCheckResultScoresThreshold();
		_collatedSpellCheckResultDisplayThreshold = GetterUtil.getInteger(
		_portletPreferences.getValue("collatedSpellCheckResultDisplayThreshold", null), collatedSpellCheckResultScoresThreshold);
		if (_collatedSpellCheckResultDisplayThreshold < 0) {
			_collatedSpellCheckResultDisplayThreshold = collatedSpellCheckResultScoresThreshold;
		}
		return _collatedSpellCheckResultDisplayThreshold;
	}
	...
	public List<SearchFacet> getEnabledSearchFacets() {
		...
		_enabledSearchFacets = ListUtil.filter(SearchFacetTracker.getSearchFacets(), new PredicateFilter<SearchFacet>() {
	...
	public String getKeywords() {
		return ParamUtil.getString(_renderRequest, SearchPortletParameterNames.KEYWORDS, null);
	}
	
	public long[] getAreeTematiche() {
		return GetterUtil.getLongValues(ParamUtil.getParameterValues(_renderRequest, "area"));
		/* SMC
		Long[] areeIds=null;
		String[] areeString=ParamUtil.getParameterValues(_renderRequest, "area");
		if(areeString!=null){
			areeIds=new Long[areeString.length];
		for(int i=0;i<areeString.length;i++){
			areeIds[i]=Long.parseLong(areeString[i]);
		}
		}
		return areeIds;
		*/
	...
	public PortletURL getPortletURL() throws PortletException {
		//		try{
		return _portletURLFactory.getPortletURL(_renderRequest, _renderResponse);
		//		} catch(Exception e) {
		//			throw new SearchContextRuntimeException(e);
		//		}
	}
	...
	public QueryConfig getQueryConfig() {
		...
		_queryConfig.setCollatedSpellCheckResultEnabled(isCollatedSpellCheckResultEnabled());
		...
		_queryConfig.setQuerySuggestionScoresThreshold(getQuerySuggestionsDisplayThreshold());
	...
	public int getQueryIndexingThreshold() {
		...
		_queryIndexingThreshold = GetterUtil.getInteger(_portletPreferences.getValue("queryIndexingThreshold", null),
	...
	public int getQuerySuggestionsDisplayThreshold() {
		...
		_querySuggestionsDisplayThreshold = GetterUtil.getInteger(_portletPreferences.getValue("querySuggestionsDisplayThreshold", null),
		_indexSearchPropsValues.getQuerySuggestionScoresThreshold());
		if (_querySuggestionsDisplayThreshold < 0) {
			_querySuggestionsDisplayThreshold = _indexSearchPropsValues.getQuerySuggestionScoresThreshold();
		}
		return _querySuggestionsDisplayThreshold;
	}
	...
	public int getQuerySuggestionsMax() {
		...
		_querySuggestionsMax = GetterUtil.getInteger(_portletPreferences.getValue("querySuggestionsMax", null),
		_indexSearchPropsValues.getQuerySuggestionMax());
		if (_querySuggestionsMax <= 0) {
			_querySuggestionsMax = _indexSearchPropsValues.getQuerySuggestionMax();
		}
		return _querySuggestionsMax;
	}
	...
	public String getSearchScopePreferenceString() {
		...
		_searchScopePreferenceString = _portletPreferences.getValue("searchScope", StringPool.BLANK);
		return _searchScopePreferenceString;
	}
	...
	public boolean isCollatedSpellCheckResultEnabled() {
		...
		_collatedSpellCheckResultEnabled = GetterUtil.getBoolean(_portletPreferences.getValue("collatedSpellCheckResultEnabled", null),
	...
	public boolean isDisplayMainQuery() {
		...
		_displayMainQuery = GetterUtil.getBoolean(_portletPreferences.getValue("displayMainQuery", null));
		return _displayMainQuery;
	}
	...
	public boolean isDisplayOpenSearchResults() {
		...
		_displayOpenSearchResults = GetterUtil.getBoolean(_portletPreferences.getValue("displayOpenSearchResults", null));
		return _displayOpenSearchResults;
	}
	...
	public boolean isDisplayResultsInDocumentForm() {
		...
		_displayResultsInDocumentForm = GetterUtil.getBoolean(_portletPreferences.getValue("displayResultsInDocumentForm", null));
		ThemeDisplay themeDisplay = getThemeDisplay();
	...
	public boolean isQueryIndexingEnabled() {
		...
		_queryIndexingEnabled = GetterUtil.getBoolean(_portletPreferences.getValue("queryIndexingEnabled", null),
	...
	public boolean isQuerySuggestionsEnabled() {
		...
		_querySuggestionsEnabled = GetterUtil.getBoolean(_portletPreferences.getValue("querySuggestionsEnabled", null),
	...
	public boolean isViewInContext() {
		...
		_viewInContext = GetterUtil.getBoolean(_portletPreferences.getValue("viewInContext", null), true);
		return _viewInContext;
	}
	...
	protected BooleanQuery createBooleanQueryCategories(SearchContext searchContext, final List<Long> categoryIds,
		final BooleanClauseOccur booleanClauseOccur)
	throws ParseException, PortalException, SystemException {
		final BooleanQuery booleanQueryCategoryId = BooleanQueryFactoryUtil.create(searchContext);
		for (final long categoryId : categoryIds) {
			booleanQueryCategoryId.addTerm(Field.ASSET_CATEGORY_IDS, String.valueOf(categoryId), false, booleanClauseOccur);
		}
		return booleanQueryCategoryId;
	}
	
	protected void addEnabledSearchFacets(long companyId, SearchContext searchContext) throws PortletException {
