...
@Component(
		configurationPid = WeatherMessageListenerConfigurator.PID,
		immediate = true,
		service = {
				WeatherMessageListener.class,
				MessageListener.class
		}
		)
public class WeatherMessageListener extends BaseMessageListener {

	public static final String GROUP_NAME = "Get Meteo Data";
	...
	protected void doReceive(Message message) throws Exception {
		if (_log.isInfoEnabled()) {
			_log.info("Start Meteo data retrieve");
		}
		...
		if (_log.isInfoEnabled()) {
			_log.info("End Meteo data retrieve");
		}

	}

	private void _getWeather() throws PortalException {
		Date date = new Date();
		Timestamp timeStamp = new Timestamp(date.getTime());
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd.HH.mm.ss");
		String url = "https://api.darksky.net/forecast/" + _configuration.getAPIKey() + "/" + _configuration.getLatitude() + "," + _configuration.getLongitude() + "?exclude=alerts,flags,hourly,minutely";
		String jsonString = callURL(url,"",80,"","");
		CachePortlet cacheRadiocor = null;
		try {
			cacheRadiocor = _cachePortletLocalService.getCachePortlet(CachePorletConstants.WEATHER_PORTLET_NAME);
		} catch (NoSuchCachePortletException e) {
			_log.error("No such CachePorlet exist with name "+CachePorletConstants.WEATHER_PORTLET_NAME+". The record will now be created"+e);
			cacheRadiocor = _cachePortletLocalService.createCachePortlet(CachePorletConstants.WEATHER_PORTLET_NAME);
		}
	...
	protected void activate(Map<String, Object> properties) {
		_configuration = Configurable.createConfigurable(
				WeatherMessageListenerConfigurator.class,
				properties);
	...
	private volatile WeatherMessageListenerConfigurator
	_configuration;
	...
	private static final Log _log = LogFactoryUtil.getLog(WeatherMessageListener.class);
