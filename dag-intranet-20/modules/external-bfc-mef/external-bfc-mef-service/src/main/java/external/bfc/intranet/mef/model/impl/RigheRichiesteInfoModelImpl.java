...
import external.bfc.intranet.mef.model.RigheRichiesteInfoModel;
import external.bfc.intranet.mef.model.RigheRichiesteInfoSoap;
...
/**
 * The base model implementation for the RigheRichiesteInfo service. Represents a row in the &quot;NEW_RIGHE_RICHIESTA_BFC_INFO&quot; database table, with each column mapped to a property of this class.
 *
 * <p>
 * This implementation and its corresponding interface {@link RigheRichiesteInfoModel} exist only as a container for the default property accessors generated by ServiceBuilder. Helper methods and all application logic should be put in {@link RigheRichiesteInfoImpl}.
 * </p>
 *
...
@JSON(strict = true)
@ProviderType
public class RigheRichiesteInfoModelImpl extends BaseModelImpl<RigheRichiesteInfo>
	implements RigheRichiesteInfoModel {
	/*
	 * NOTE FOR DEVELOPERS:
	 *
	 ...
	 */
	public static final String TABLE_NAME = "NEW_RIGHE_RICHIESTA_BFC_INFO";
	...
	public static final String TABLE_SQL_CREATE = "create table NEW_RIGHE_RICHIESTA_BFC_INFO (ID LONG not null,ID_MATERIALE LONG not null,NUMERO_RIGA LONG,QUANTITA_RICHIESTA LONG,QUANTITA_DISPONIBILE LONG,UID_INS LONG,DATA_INS DATE null,UID_UPD LONG,DATA_UPD DATE null,IMPORTO DOUBLE,SOTTOGRUPPO VARCHAR(75) not null,GRUPPO VARCHAR(75) not null,DESCRIZIONE VARCHAR(75) null,primary key (ID, ID_MATERIALE, SOTTOGRUPPO, GRUPPO))";
	public static final String TABLE_SQL_DROP = "drop table NEW_RIGHE_RICHIESTA_BFC_INFO";
	public static final String ORDER_BY_JPQL = " ORDER BY righeRichiesteInfo.id.id ASC, righeRichiesteInfo.id.idMateriale ASC, righeRichiesteInfo.id.sottogruppo ASC, righeRichiesteInfo.id.gruppo ASC";
	public static final String ORDER_BY_SQL = " ORDER BY NEW_RIGHE_RICHIESTA_BFC_INFO.ID ASC, NEW_RIGHE_RICHIESTA_BFC_INFO.ID_MATERIALE ASC, NEW_RIGHE_RICHIESTA_BFC_INFO.SOTTOGRUPPO ASC, NEW_RIGHE_RICHIESTA_BFC_INFO.GRUPPO ASC";
	...
	public static final boolean ENTITY_CACHE_ENABLED = GetterUtil.getBoolean(external.bfc.intranet.mef.service.util.PropsUtil.get(
				"value.object.entity.cache.enabled.external.bfc.intranet.mef.model.RigheRichiesteInfo"),
			true);
	public static final boolean FINDER_CACHE_ENABLED = GetterUtil.getBoolean(external.bfc.intranet.mef.service.util.PropsUtil.get(
				"value.object.finder.cache.enabled.external.bfc.intranet.mef.model.RigheRichiesteInfo"),
			true);
	public static final boolean COLUMN_BITMASK_ENABLED = GetterUtil.getBoolean(external.bfc.intranet.mef.service.util.PropsUtil.get(
				"value.object.column.bitmask.enabled.external.bfc.intranet.mef.model.RigheRichiesteInfo"),
			true);
	 ...
	 */
	public static RigheRichiesteInfo toModel(RigheRichiesteInfoSoap soapModel) {
		if (soapModel == null) {
			return null;
		}

		RigheRichiesteInfo model = new RigheRichiesteInfoImpl();
	 ...
	 */
	public static List<RigheRichiesteInfo> toModels(
		RigheRichiesteInfoSoap[] soapModels) {
		if (soapModels == null) {
			return null;
		}

		List<RigheRichiesteInfo> models = new ArrayList<RigheRichiesteInfo>(soapModels.length);

		for (RigheRichiesteInfoSoap soapModel : soapModels) {
	...
	public static final long LOCK_EXPIRATION_TIME = GetterUtil.getLong(external.bfc.intranet.mef.service.util.PropsUtil.get(
				"lock.expiration.time.external.bfc.intranet.mef.model.RigheRichiesteInfo"));

	public RigheRichiesteInfoModelImpl() {
	}
	...
	public void setPrimaryKeyObj(Serializable primaryKeyObj) {
		setPrimaryKey((RigheRichiesteInfoPK)primaryKeyObj);
	}
	...
	@Override
	public RigheRichiesteInfo toEscapedModel() {
		if (_escapedModel == null) {
			_escapedModel = (RigheRichiesteInfo)ProxyUtil.newProxyInstance(_classLoader,
	...
	public Object clone() {
		...
		righeRichiesteInfoImpl.setId(getId());
		righeRichiesteInfoImpl.setIdMateriale(getIdMateriale());
		righeRichiesteInfoImpl.setNumeroRiga(getNumeroRiga());
		righeRichiesteInfoImpl.setQuantitaRichiesta(getQuantitaRichiesta());
		righeRichiesteInfoImpl.setQuantitaDisponibile(getQuantitaDisponibile());
		righeRichiesteInfoImpl.setUidIns(getUidIns());
		righeRichiesteInfoImpl.setDataIns(getDataIns());
		righeRichiesteInfoImpl.setUidUpd(getUidUpd());
		righeRichiesteInfoImpl.setDataUpd(getDataUpd());
		righeRichiesteInfoImpl.setImporto(getImporto());
		righeRichiesteInfoImpl.setSottogruppo(getSottogruppo());
		righeRichiesteInfoImpl.setGruppo(getGruppo());
		righeRichiesteInfoImpl.setDescrizione(getDescrizione());
	...
	@Override
	public int compareTo(RigheRichiesteInfo righeRichiesteInfo) {
		RigheRichiesteInfoPK primaryKey = righeRichiesteInfo.getPrimaryKey();
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof RigheRichiesteInfo)) {
			return false;
		}

		RigheRichiesteInfo righeRichiesteInfo = (RigheRichiesteInfo)obj;
	...
	public void resetOriginalValues() {
		RigheRichiesteInfoModelImpl righeRichiesteInfoModelImpl = this;

		righeRichiesteInfoModelImpl._originalId = righeRichiesteInfoModelImpl._id;

		righeRichiesteInfoModelImpl._setOriginalId = false;

		righeRichiesteInfoModelImpl._columnBitmask = 0;
	}

	@Override
	public CacheModel<RigheRichiesteInfo> toCacheModel() {
		RigheRichiesteInfoCacheModel righeRichiesteInfoCacheModel = new RigheRichiesteInfoCacheModel();

		righeRichiesteInfoCacheModel.righeRichiesteInfoPK = getPrimaryKey();

		righeRichiesteInfoCacheModel.id = getId();

		righeRichiesteInfoCacheModel.idMateriale = getIdMateriale();

		righeRichiesteInfoCacheModel.numeroRiga = getNumeroRiga();

		righeRichiesteInfoCacheModel.quantitaRichiesta = getQuantitaRichiesta();

		righeRichiesteInfoCacheModel.quantitaDisponibile = getQuantitaDisponibile();

		righeRichiesteInfoCacheModel.uidIns = getUidIns();

		Date dataIns = getDataIns();

		if (dataIns != null) {
			righeRichiesteInfoCacheModel.dataIns = dataIns.getTime();
		}
		else {
			righeRichiesteInfoCacheModel.dataIns = Long.MIN_VALUE;
		}

		righeRichiesteInfoCacheModel.uidUpd = getUidUpd();

		Date dataUpd = getDataUpd();

		if (dataUpd != null) {
			righeRichiesteInfoCacheModel.dataUpd = dataUpd.getTime();
		}
		else {
			righeRichiesteInfoCacheModel.dataUpd = Long.MIN_VALUE;
		}

		righeRichiesteInfoCacheModel.importo = getImporto();

		righeRichiesteInfoCacheModel.sottogruppo = getSottogruppo();

		String sottogruppo = righeRichiesteInfoCacheModel.sottogruppo;

		if ((sottogruppo != null) && (sottogruppo.length() == 0)) {
			righeRichiesteInfoCacheModel.sottogruppo = null;
		}

		righeRichiesteInfoCacheModel.gruppo = getGruppo();

		String gruppo = righeRichiesteInfoCacheModel.gruppo;

		if ((gruppo != null) && (gruppo.length() == 0)) {
			righeRichiesteInfoCacheModel.gruppo = null;
		}

		righeRichiesteInfoCacheModel.descrizione = getDescrizione();

		String descrizione = righeRichiesteInfoCacheModel.descrizione;

		if ((descrizione != null) && (descrizione.length() == 0)) {
			righeRichiesteInfoCacheModel.descrizione = null;
		}

		return righeRichiesteInfoCacheModel;
	}
	...
	public String toXmlString() {
		...
		sb.append("external.bfc.intranet.mef.model.RigheRichiesteInfo");
	...
	private static final ClassLoader _classLoader = RigheRichiesteInfo.class.getClassLoader();
	private static final Class<?>[] _escapedModelInterfaces = new Class[] {
			RigheRichiesteInfo.class
		};
	private long _id;
	...
	private RigheRichiesteInfo _escapedModel;
}