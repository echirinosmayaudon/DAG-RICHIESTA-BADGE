...
import com.mef.pdfbox.custom.rotate.PDFCustomGeneric;
import com.mef.pdfbox.custom.rotate.PDFCustomPageLayout;
import com.mef.pdfbox.custom.rotate.PDFCustomTable;
import com.mef.pdfbox.custom.rotate.PDFCustomTableCell;
import com.mef.pdfbox.custom.rotate.PDFCustomTableRow;
import com.mef.pdfbox.custom.rotate.PDFCustomText;
...
public enum StampaReportDay {
	...
	static final Log _log = LogFactoryUtil.getLog(StampaReportDay.class);
	...
	public static void drawPDFStampaDettagliata(ResourceResponse resourceResponse, ResourceRequest resourceRequest,
		...
		try {
			...
			for (int i = 0; i < content.size(); i++) {
				pdfCustomStampa.writeTextPDF(
						numero_riunione + ". Riunione del " + content.get(i).get(StampaReportDay.CONSTANT_GIORNO_INIZIO),
						font, fontSizeHeader);

				pdfCustomStampa.writeTextPDF("- Ora inizio: " + content.get(i).get(StampaReportDay.CONSTANT_ORA_INIZIO),
						font, fontSizeContainer);
				pdfCustomStampa.writeTextPDF(
						"- Inserita in E-ROOM il: " + content.get(i).get(StampaReportDay.CONSTANT_E_ROOM), font,
						fontSizeContainer);
				pdfCustomStampa.writeTextPDF("- Oggetto: " + content.get(i).get(StampaReportDay.CONSTANT_OGGETTO), font,
						fontSizeContainer);
				pdfCustomStampa.writeTextPDF("- Luogo: " + content.get(i).get(StampaReportDay.CONSTANT_LUOGO), font,
						fontSizeContainer);
				pdfCustomStampa.writeTextPDF("- Tipo: " + content.get(i).get(StampaReportDay.CONSTANT_TIPO), font,
						fontSizeContainer);

				pdfCustomStampa.writeTextPDF(
						"- Durata Prevista: " + content.get(i).get(StampaReportDay.CONSTANT_DURATA_PREVISTA), font,
						fontSizeContainer);

				pdfCustomStampa.writeTextPDF("- Convocante: " + content.get(i).get(StampaReportDay.CONSTANT_CONVOCANTE),
						font, fontSizeContainer);
				pdfCustomStampa.writeTextPDF("- Stato: " + content.get(i).get(StampaReportDay.CONSTANT_STATO_RIUNIONE),
						font, fontSizeContainer);
				pdfCustomStampa.writeTextPDF("- Protocollo: " + content.get(i).get(StampaReportDay.CONSTANT_PROTOCOLLO),
						font, fontSizeContainer);
				pdfCustomStampa.writeTextPDF("- Note coordinatore: " + content.get(i).get(StampaReportDay.CONSTANT_NOTE),
						font, fontSizeContainer);
				pdfCustomStampa.addLine(10);

				List<InvitatiRiunione> segreterie = InvitatiRiunioneLocalServiceUtil
						.getInvitatiRiunioneByIdRiunione(Long.parseLong(content.get(i).get(StampaReportDay.CONSTANT_ID)));
				...
				if (segreterie != null && !segreterie.isEmpty()) {
					...
					cellsHeader.add(new PDFCustomTableCell("Partecipanti", PDType1Font.HELVETICA_BOLD, fontSizeTable, 7,
							true, true));
					cellsHeader.add(new PDFCustomTableCell(StampaReportDay.CONSTANT_NOTE.getNome(),
							PDType1Font.HELVETICA_BOLD, fontSizeTable, 7, true, true));

					cellsHeader.add(new PDFCustomTableCell(StampaReportDay.CONSTANT_STATO_RIUNIONE.getNome(),
							PDType1Font.HELVETICA_BOLD, fontSizeTable, 3, true, true));
					cellsHeader.add(new PDFCustomTableCell(StampaReportDay.CONSTANT_ISPETTORATO.getNome(),
	...
	private static void drawPDFStampa(List<Map<StampaReportDay, String>> content, ResourceResponse resourceResponse,
		...
		try {
			...
			// createHeader campi
			List<PDFCustomTableCell> cellsHeader = new ArrayList<>();
			for (StampaReportDay s : content.get(0).keySet()) {
				if (s.getSizeCol() > 0)
					if ((flagGiornaliera && (!s.equals(StampaReportDay.CONSTANT_GIORNO_INIZIO)))|| !flagGiornaliera)
			...
			for (int i = 0; i < content.size(); i++) {
				List<PDFCustomTableCell> cells = new ArrayList<>();
				for (StampaReportDay s : content.get(i).keySet()) {
					if (s.getSizeCol() > 0){
						if(s.equals(StampaReportDay.CONSTANT_ISPETTORATO)){
							...
							for(InvitatiRiunione invitatiSegreterie : segreterie){
								...
								if(segreteriaInvitata.getListPartecipante()!=null && !segreteriaInvitata.getListPartecipante().isEmpty()){
									...
									for(Partecipante partecipante: segreteriaInvitata.getListPartecipante()){
										...
										pdfCustomTextPartecipante.setFontSize(fontSizeFields-1);
	...
	public static void drawPDFStampa(List<Map<StampaReportDay, String>> content, ResourceResponse resourceResponse,
	...
	public static void drawPDFStampaGiornaliera(List<Map<StampaReportDay, String>> content,
