...
public class PDFCustomGeneric {
	...
	private void drawCellTextPDF(PDFCustomTableCell cell, float positionX, float positionY) {
		...
		float movePdf = (cell.getHeightTextPDF() / (cell.getTextPDF().size()));
		...
		//if (cell.getTextPDF().size() > 1)
			pos -= (move * (cell.getTextPDF().size()));
		pos+=movePdf;
		int size = cell.getTextPDF().size();
		if(cell.getCustomText()!=null){
			size=  cell.getCustomTextList().size();
		}
		float y;
		float x;
		for (int i = 0; i < size; i++) {
			...
			if (cell.getCustomText()==null){
				this.drawTextPDF(cell.getTextPDF().get(i), x, y, cell.getFontSize(), cell.getFontFamily(), this.rotateText);
				try {
					if (cell.isUnderline()){
						if (this.rotatePage)
							contentStream.drawLine(x+1, y, x+1, y+cell.getMinWidthPDF(i));				
						else
							contentStream.drawLine(x, y+1, x-cell.getMinWidthPDF(i), y+1);
	...
	private float drawRowsPDF(PDFCustomTable table, float heightOff, float widthOffset, float heightOverflow,
		...
		try {
			...
			for (PDFCustomTableRow row : rows) {
				width = widthOffset;
				row.calculateRow();
