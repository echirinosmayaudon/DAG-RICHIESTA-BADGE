...
<c:choose>
	<c:when test="<%= isSocialCollaborationCompany %>">
		<liferay-util:include page="/search_smc.jsp" servletContext="<%= application %>"/>
	</c:when>
	<c:when test="<%=  !isSocialCollaborationCompany && themeDisplay.getScopeGroup().isControlPanel()%>">
		<liferay-util:include page="/search.original.jsp" useCustomPage="false" />
	</c:when>
	<c:otherwise>
		<liferay-util:include page="/search_mef.jsp" servletContext="<%= application %>"/>
	</c:otherwise>
</c:choose>