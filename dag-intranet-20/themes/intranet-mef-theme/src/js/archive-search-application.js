...
function showHideFilterBar(){
    var assetArchiveJs = $(".asset-archive-js-app");
...
function searchCategory (btn){
    if(btn.closest(".asset-archive-js-app").find('.filter-category-item-box.active').length > 0){
        ...
        var urlLocation = location.origin+location.pathname;
        var categoryArray = [];
        btn.closest(".asset-archive-js-app").find(".filtro-body-box .filter-category-item-box.active").each(function(){
            var categoryId = $(this).data("category-id");
            categoryArray.push(categoryId);
        });
        var categorySearch = "_categories="+categoryArray.join(",");
        if(location.href.indexOf("keywords=") ==-1 && btn.closest(".asset-archive-js-app").find('.text-search-wrap .text-search').val() == ""){
            window.location = urlLocation+"?_" + portletId +categorySearch+costantsUrl;
        }
        else if(btn.closest(".asset-archive-js-app").find('.text-search-wrap .text-search').val() != ""){
            var keywords = btn.closest(".asset-archive-js-app").find('.text-search-wrap .text-search').val();
        ...
        else if(location.href.indexOf("keywords=") !=-1 && btn.closest(".asset-archive-js-app").find('.text-search-wrap .text-search').val() == ""){
...
function searchKeywords(btn){
    if(btn.closest(".asset-archive-js-app").find('.text-search-wrap .text-search').val() != ""){
        var portletId = btn.closest("section").attr("id").substring(8);
        var ppid = "p_p_id=" + portletId;
        var pplifecycle = "p_p_lifecycle=0";
        var ppstate = "p_p_state=normal";
